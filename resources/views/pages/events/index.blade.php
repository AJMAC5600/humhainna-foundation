    ...
    <div class="block sm:hidden w-full">
        <div class="w-full px-5 pt-5 pb-5 text-center">
            ...
            <p class="max-w-[340px] mx-auto text-[11px] leading-[1.35] text-slate-700">
                Join an upcoming event or browse the impact
                of our past drives.
            </p>
        ...
        <img src="{{ asset('images/hero-2mobile.png') }}"
             alt="Hum Hain Na Foundation — Events"
...
<div class="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop
    ...
    <div class="flex flex-wrap gap-2 sm:gap-3">
        <a href="{{ route('events.index', ['filter' => 'upcoming']) }}"
           class="font-label-sm text-label-sm
                  ...
                  {{ $filter !== 'past'
                        ? 'bg-[#102957] text-white shadow-sm font-semibold'
    ...
    <div>
            ...
            @forelse ($events as $event)
                <a href="{{ route('events.show', $event) }}"
                   class="group bg-surface rounded-2xl overflow-hidden
                    ...
                    <div class="aspect-[4/3] relative overflow-hidden bg-gradient-to-br from-primary-container to-surface-tint">
                        ...
                        @else
                            <div class="w-full h-full flex items-center justify-center">
                                <span class="material-symbols-outlined text-4xl text-white/60">event</span>
                            </div>
                        @endif

                        {{-- Category --}}
                        <span class="absolute top-3 left-3 sm:top-4 sm:left-4
                                     capitalize bg-surface/90 backdrop-blur-sm
                                     px-2.5 sm:px-3 py-1 rounded-full
                                     border border-outline-variant/30
                                     font-label-sm text-label-sm text-[10px] sm:text-xs">
                        ...
                        @if ($event->status === 'completed')
                            <span class="absolute top-3 right-3 sm:top-4 sm:right-4
                                         ...
                                         px-2.5 sm:px-3 py-1 rounded-full
                                         font-label-sm text-label-sm text-[10px] sm:text-xs font-semibold">
                                Completed
                            </span>
                        @endif
                    </div>
                    ...
                    <div class="p-5 sm:p-6 flex flex-col flex-grow">
                        ...
                        <p class="font-label-sm text-label-sm text-on-surface-variant
                                  flex items-center gap-2 mb-1.5 text-xs sm:text-sm">
                            <span class="material-symbols-outlined text-base shrink-0">calendar_month</span>
                            <span class="truncate">
                                {{ $event->starts_at->format('D, d M Y') }}{{ $event->ends_at ? ' – '.$event->ends_at->format('d M Y') : '' }}
                            </span>
                        </p>

                        <p class="font-label-sm text-label-sm text-on-surface-variant
                                  flex items-center gap-2 flex-grow text-xs sm:text-sm">
                            <span class="material-symbols-outlined text-base shrink-0">location_on</span>
                            <span class="truncate">{{ Str::limit($event->location, 60) }}</span>
                        </p>

                        @if ($event->volunteers_required)
                            <p class="mt-4 inline-flex w-fit items-center gap-1.5
                                      bg-secondary-fixed-dim/40 text-secondary
                                      font-label-sm text-label-sm text-xs sm:text-sm
            ...
            @empty
                <div class="col-span-full bg-surface-container-low rounded-2xl
                    ...
                    <div class="w-16 h-16 rounded-full bg-surface-container-high mx-auto
                        ...
                        <span class="material-symbols-outlined text-3xl text-outline-variant">event_busy</span>
                    </div>
                    <p class="font-body-lg text-body-lg text-on-surface-variant">
                        {{ $filter === 'past'
                            ? 'No past events published yet.'
                            : 'No upcoming events right now — follow us on social media for announcements.' }}
                    </p>
                </div>
            @endforelse
        ...
        <div class="mt-10 md:mt-12">{{ $events->links() }}</div>
    </div>
