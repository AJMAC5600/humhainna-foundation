...
@section('content')
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16 md:mb-24">
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5" id="counters">

            @forelse (\App\Models\Achievement::counters()->get() as $c)
                <div class="group bg-white p-5 sm:p-7 rounded-2xl shadow-sm border border-slate-200/60
                                                                        text-center hover:shadow-lg hover:-translate-y-1 hover:border-amber-200
                                                                        transition-all duration-300">

                    <div
                        class="w-12 h-12 sm:w-14 sm:h-14 mx-auto rounded-xl bg-blue-50 flex items-center justify-center mb-3 sm:mb-4
                                                                            group-hover:bg-amber-100 transition-colors duration-300">
                        <span
                            class="material-symbols-outlined text-2xl sm:text-3xl text-blue-900 group-hover:text-amber-600 transition-colors duration-300">
                            {{ $c->icon ?? 'favorite' }}
                        </span>
                    </div>

                    <div class="text-3xl sm:text-4xl lg:text-5xl font-bold text-blue-900 counter-value leading-none"
                        data-target="{{ preg_replace('/[^0-9]/', '', $c->value ?? '0') }}">
                        {{ $c->value }}
                    </div>

                    <span class="block mt-2 text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-slate-500">
                        {{ $c->title }}
                    </span>
                </div>
            @empty
